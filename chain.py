
import hashlib
from block import Block

class Chain():
    def __init__(self, difficulty) -> None:
        self.difficulty = difficulty
        self.blocks = []
        self.pool = []
        self.create_origin_block()
    
    def proof_of_work(self, block):
        hash = hashlib.sha512()
        hash.update(str(block).encode('utf-8'))
        return block.hash.hexdigest() == hash.hexdigest() and int(hash.hexdigest(), 16) < 2**(512-self.difficulty) and block.previous_hash == self.blocks[-1].hash
    
    def add_to_chain(self, block):
        if self.proof_of_work(block):
            self.blocks.append(block)
    
    def add_to_pool(self, data):
        self.pool.append(data)
    
    def create_origin_block(self):
        hash = hashlib.sha512()
        hash.update(''.encode('utf-8'))
        origin = Block("origin", hash)
        origin.mine(self.difficulty)
        self.blocks.append(origin)
    
    def mine(self):
        if len(self.pool) > 0:
            data = self.pool.pop()
            block = Block(data, self.blocks[-1].hash)
            block.mine(self.difficulty)
            self.add_to_chain(block)
            print("\n==============================")
            print("Hash: {}".format(block.hash.hexdigest()))
            print("Previous hash: {}".format(block.previous_hash.hexdigest()))
            print("Nonce: {}".format(block.nonce))
            print("Data: {}".format(block.data))
            print("\n==============================")
